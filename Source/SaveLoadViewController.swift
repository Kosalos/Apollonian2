...
class SaveLoadViewController: UIViewController,UITableViewDataSource, UITableViewDelegate,SLCellDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if dateString == "**" {
            str = "** unused **"
            cell.loadCell.backgroundColor = UIColor(red:0.1, green:0.1, blue:0.3, alpha:1)
        }
        else {
            str = String(format:"%2d  %@", index,dateString)
            cell.loadCell.backgroundColor = UIColor(red:0.2, green:0.4, blue:0.2, alpha:1)
        }
    ...
    func didTapButton(_ sender: UIButton) {
        ...
        if let indexPath = getCurrentCellIndexPath(sender) {
            ...
            if sender.tag == 0 { loadAndDismissDialog(indexPath.row,&vc.control) }
            if sender.tag == 1 { saveAndDismissDialog(indexPath.row,vc.control) }
        }
    }
